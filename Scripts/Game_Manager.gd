...
func _ready():
	# Inicia la generación de oxígeno periódica
	call_deferred("start_oxygen_spawn")

func _process(delta: float) -> void:
	update_organ_logic(delta)
	check_victory(delta)

# Lógica de generación de oxígeno
func start_oxygen_spawn() -> void:
	while true:
		await get_tree().create_timer(oxygen_spawn_rate).timeout
		print("Generando oxígeno...")

# Actualización de las funcionalidades específicas de los órganos
func update_organ_logic(delta: float) -> void:
	update_kidneys()
	update_liver()
	update_stomach()
	update_pancreas()
